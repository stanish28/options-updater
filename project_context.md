...
**Login timeout cap (built 2026-09-21):** `robin_stocks`' device-approval flow (`authentication._validate_sherrif_id`) polls in an **unbounded** `while True: time.sleep(5)` loop. On an unattended cron run against an expired session nobody ever taps "approve", so the run hung **~50 minutes** until a network error killed it — delaying the failure alert by the same ~50 min (observed 2026-09-18). `login()` now wraps `r.login()` in a **SIGALRM cap** (`LOGIN_TIMEOUT_SECONDS`, default **120s**) raising `LoginTimeout`, which the `__main__` handler turns into an immediate alert + non-zero exit. Do **not** set this too low: a *healthy* cached login still takes ~14s, so a tight cap causes false failures. Patching the library directly was rejected — a `pip install` would wipe it; the signal cap is version-proof.
